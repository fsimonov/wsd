...
if len (sys.argv) < 2:
    ...
    print ("Format: python wsd.py word_vectors.bin [context_vectors.w2v]")
    sys.exit (1)
...
word_vectors = load_vectors("{}".format(sys.argv[1]))
print ("Word vectors loaded")
if len (sys.argv) == 3:
    context_vectors = load_vectors("{}".format(sys.argv[2]))
    print ("Context vectors loaded")
else:
    context_vectors = word_vectors

listOfSets = ['./data/TWSI_dev.csv', './data/Dataset-TWSI-2-GOLD.csv.gz']
listOfInv = ['./data/Inventory-59g-agagram-alpha-15.csv.gz', './data/Inventory-59g-agagram-alpha-20.csv.gz', './data/Inventory-59g-agagram-alpha-75.csv.gz']

#FR = './results6.csv'
#df = pd.DataFrame(columns = ['Precision', 'Recall', 'F1'])
#df.to_csv(FR, sep = '\t', encoding='utf8')

for FSET in listOfSets:
    for FINV in listOfInv:
        for gamma in np.linspace(0.2, 0.8, 13):
            ...
            clf.predict(df, gamma)
            print "%.2f%% completed" % float(100)
            print ("Senses predicted")

            FRES = "{}{}{}{}{}{}{}".format(delete_substring(FSET, '.csv'), '_', name_of_file(delete_substring(FINV, '.csv')), '_', name_of_file(delete_substring(delete_substring(sys.argv[1], '.bin'), '.w2v')), "_gamma_%.2f" % gamma, "_predicted.csv")
            df.to_csv(FRES, sep = '\t', encoding='utf8')
            print ("Results are in {}".format(FRES))

            cmd = "python twsi_evaluation.py {} {}".format(FINV, FRES)
